      ...
      //  qDebug()<<"loop...";
        if(this->tokens[i]->type==Token2::TypeChildMarker){
            ...
            if(QString::compare(this->tokens[i]->_string,";")==0){
                ...
                else if(QString::compare(this->tokens[i]->_string,"if")==0){
                  //  ifs.append(codes.size());
...
QVector<QVector<Token2*>> LexNode::reorderTokenStream(bool isLambda,bool isIfStatement,bool isWhileStatement,int &lexicalId,QVector<QString> args){
    int i=0;
        ...
        while(e<this->data[i].size()){
           ...
           qDebug()<<"inner loop...on token:"<<this->data[i][e]->_string;
            if(this->data[i][e]->type!=Token2::TypeChildMarker && !(e==0 && (QString::compare(this->data[i][e]->_string,"let")==0 ||
                ...
                subExpressions = this->childs[this->data[i][e]->data]->reorderTokenStream(isLam,isIf,isWhile,lexId,newData[lambdaAddressI][lambdaAddressE]->expectedArguments);
...
PByteArray* LexNode::compile(bool top){
    ...
    if(top){
        ...
        int discard=0;
        this->reorderTokenStream(!top,false,false,discard,{});
        this->precompile(top);
        LexNode::HighestRPN++;
