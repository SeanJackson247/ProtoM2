    ...
    while(i<data.size()){
        ...
        if(i==0){
            ...
            if(QString::compare(data[i]->_string,"[")==0 && data[i-1]->isOperator()==true){
                ...
                res.removeLast();
                res.append(new Token2(Token2::TypeCode,"ARRAY_LOOK_UP"));
                res.append(data[i]);
            }
        }
        i++;
...
QVector<Token2*> LineHelper::removeLambdaArgs2(QVector<Token2*> data){
    ...
    while(i<data.size()){
        if(inLambda==false){
            if(QString::compare("lambda",data[i]->_string)==0){
                inLambda = true;
            }
            res.append(data[i]);
        }
                            ...
                            do{
                               ...
                               // currentPrecedence = operatorStack.last()->operatorPrecedence();
                                if(isRightBracket==true && QString::compare(operatorStack.last()->_string,"(")==0){
                                    ...
                                    if(operatorStack.size()>0 && (operatorStack.last()->type==Token::TypeInvocationMarker)  && (QString::compare(operatorStack.last()->_string,"$")==0)){
...
/*                                else if(QString::compare(operatorStack.last()->_string,",")==0 &&
                                        QString::compare(data[i]->_string,",")==0){
                                    operandStack.append(operatorStack.last());
                                    operatorStack.removeLast();
                                    if(operatorStack.size()>0){
