...
/*        int newBAddress = this->data->size();
    ...
    if(A_Type==ByteCodeInstruction::TypeArrayType || A_Type==ByteCodeInstruction::TypeArgsListType){
        if(B_Type==ByteCodeInstruction::TypeArgsListType){
            ...
            while(i<bSize){
                int entryAddress = (Architecture::TypeArrayGutter)+(i*Architecture::TypeArrayEntrySize);
            ...
            if(A_Type==ByteCodeInstruction::TypeArrayType){
            ...
            if(size==capacity){
                //////this->debuggerOut("Appending to array or args list, BAddress:"+QString::number(B_Address)+" size==capacity",true);
                ...
                //////this->debuggerOut("Appending to array or args list, BAddress:"+QString::number(B_Address)+" size is less than capacity",true);
...
int Heap::completeDeref(int address){
    ...
    while(address>Architecture::SIZE_OF_INITIAL_HEAP_STORE &&  type==ByteCodeInstruction::TypePropertyKeyType || type==ByteCodeInstruction::TypeHeapFragment || type==ByteCodeInstruction::TypeKeyType || type==ByteCodeInstruction::TypeArrayIndexType){
        if(type==ByteCodeInstruction::TypePropertyKeyType){
            //////this->debuggerOut("Dereferencing property key to value slot from:"+QString::number(address),true);
            address = this->decodeRawIntFrom(address+Architecture::TypePropertyKeyGutter);
            //////this->debuggerOut("Dereferenced property key to value slot to:"+QString::number(address),true);
            if(address<=Architecture::SIZE_OF_INITIAL_HEAP_STORE){
                return address;
            }
            address = this->decodeRawIntFrom(address);
            type = this->data->getByte(address);
            //////this->debuggerOut("Dereferenced property key to type:"+QString::number(type)+" at address:"+QString::number(address),true);
            if(address<=Architecture::SIZE_OF_INITIAL_HEAP_STORE){
                return address;
            }
        }
            ...
            //////this->debuggerOut("Derefed key:"+aString+", to addess:"+QString::number(address),true);
            if(address>=Architecture::SIZE_OF_INITIAL_HEAP_STORE){
                address = this->decodeRawIntFrom(address);
                type = this->data->getByte(address);
            }
...
void Heap::prependToArray(int arrayAddress,int valueAddress){
    if(this->data->getByte(arrayAddress)==ByteCodeInstruction::TypeArrayType){
...
void Heap::removeFromArray(int arrayAddress,int index){
    ...
    else if(this->data->getByte(arrayAddress)==ByteCodeInstruction::TypeArrayType){
...
void Heap::constructNewArgsList(int aAddress,int bAddress){
    ...
    //this->debuggerOut("Constructing arguments list from :"+QString::number(this->data->getByte(aAddress))+", at "+QString::number(aAddress)+" and "+QString::number(this->data->getByte(bAddress))+", at "+QString::number(bAddress)+" on line:"+QString::number(this->interpreter->lineNumber),true);
    ...
    if(this->data->getByte(bAddress)==ByteCodeInstruction::TypeConstIntegerType){
        ...
        //this->debuggerOut("B is constant string",true);
        PByteArray* _aInt = new PByteArray();
...
void Heap::constructNewArray(int aAddress){
    unsigned char typeBit = ByteCodeInstruction::TypeArrayType;
    int capacity = Architecture::TypeArgumentsListDefaultCapacity;
...
QString Heap::arrayToString(int address){
    QString res="[";// = "[ARRAY][";
    int size = decodeRawIntFrom(address+Architecture::TypeArrayGutterToSize);
    int cap = decodeRawIntFrom(address+Architecture::TypeArrayGutterToCapacity);
    int i=0;
    address+=Architecture::TypeArrayGutter;
    while(i<size){
        ...
        else if(this->data->getByte(valAddress)==ByteCodeInstruction::TypeArrayType){
            res+="array";
        }
